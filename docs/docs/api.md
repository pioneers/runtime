...
- [Input Classes](#input-classes)
  _ [`Gamepad.get_value(name_of_input)`](#gamepadget_valuename_of_input)
  _ [`Keyboard.get_value(name_of_key)`](#keyboardget_valuename_of_key)
...
def teleop_main():
  if Gamepad.get_value(button_a) and not Robot.is_running(robot_actions): # if the button A is pressed down and robot_actions are not running. Then the robot will be able to run robot_actions again. Will not run if the button A is not pressed or if robot_actions is running
