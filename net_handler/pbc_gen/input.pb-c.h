...
typedef enum _Source {
  SOURCE__GAMEPAD = 0,
  SOURCE__KEYBOARD = 1
    PROTOBUF_C__FORCE_ENUM_TO_BE_INT_SIZE(SOURCE)
} Source;
...
void   input__init
                     (Input         *message);
size_t input__get_packed_size
                     (const Input   *message);
size_t input__pack
                     (const Input   *message,
                      uint8_t             *out);
size_t input__pack_to_buffer
                     (const Input   *message,
...
Input *
       input__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   input__free_unpacked
                     (Input *message,
                      ProtobufCAllocator *allocator);
/* UserInputs methods */
void   user_inputs__init
                     (UserInputs         *message);
size_t user_inputs__get_packed_size
                     (const UserInputs   *message);
size_t user_inputs__pack
                     (const UserInputs   *message,
                      uint8_t             *out);
size_t user_inputs__pack_to_buffer
                     (const UserInputs   *message,
                      ProtobufCBuffer     *buffer);
UserInputs *
       user_inputs__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   user_inputs__free_unpacked
                     (UserInputs *message,
                      ProtobufCAllocator *allocator);
/* --- per-message closures --- */

typedef void (*Input_Closure)
                 (const Input *message,
                  void *closure_data);
typedef void (*UserInputs_Closure)
                 (const UserInputs *message,
                  void *closure_data);
