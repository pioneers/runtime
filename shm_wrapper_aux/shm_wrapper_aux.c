...
void shm_aux_init (process_t process)
{
	...
	if (process == NET_HANDLER) {
		...
		if ((gp_sem = sem_open(GP_MUTEX_NAME, O_CREAT, 0660, 0)) == SEM_FAILED) {
			log_printf(FATAL, "sem_open: gamepad_mutex@net_handler");
			exit(1);
		}
		gp_val = 0;
		...
		if ((rd_sem = sem_open(RD_MUTEX_NAME, O_CREAT, 0660, 1)) == SEM_FAILED) {
			log_printf(FATAL, "robot_desc_mutex@net_handler");
			exit(1);
		}
		rd_val = 1;
		...
		if ((fd_shm = shm_open(GPAD_SHM_NAME, O_RDWR | O_CREAT, 0660)) == -1) {
			log_printf(FATAL, "shm_open gamepad_shm: @net_handler");
			exit(1);
		}
		if (ftruncate(fd_shm, sizeof(gamepad_t)) == -1) {
			log_printf(FATAL, "ftruncate gamepad_shm: @net_handler");
			exit(1);
		}
		if ((gp_ptr = mmap(NULL, sizeof(gamepad_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd_shm, 0)) == MAP_FAILED) {
			log_printf(FATAL, "mmap gamepad_shm @net_handler");
			exit(1);
		}
		if (close(fd_shm) == -1) {
			log_printf(ERROR, "close gamepad_shm: @net_handler");
		}
		
		//create robot description shm block
		if ((fd_shm = shm_open(ROBOT_DESC_SHM_NAME, O_RDWR | O_CREAT, 0660)) == -1) {
			log_printf(FATAL, "shm_open robot_desc_shm: @net_handler");
			exit(1);
		}
		if (ftruncate(fd_shm, sizeof(robot_desc_t)) == -1) {
			log_printf(FATAL, "ftruncate robot_desc_shm: @net_handler");
			exit(1);
		}
		if ((rd_ptr = mmap(NULL, sizeof(robot_desc_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd_shm, 0)) == MAP_FAILED) {
			log_printf(FATAL, "mmap robot_desc_shm @net_handler");
			exit(1);
		}
		if (close(fd_shm) == -1) {
			log_printf(ERROR, "close robot_desc_shm: @net_handler");
		}
		...
		if ((gp_sem = sem_open(GP_MUTEX_NAME, 0, 0, 0)) == SEM_FAILED) {
			log_printf(FATAL, "sem_open: gamepad_mutex@client");
			exit(1);
		}
		
		//mutual exclusion semaphore, rd_sem
		if ((rd_sem = sem_open(RD_MUTEX_NAME, 0, 0, 0)) == SEM_FAILED) {
			log_printf(FATAL, "sem_open: robot_desc_mutex@client");
			exit(1);
		}
		...
		if ((fd_shm = shm_open(GPAD_SHM_NAME, O_RDWR, 0)) == -1) { //no O_CREAT
			log_printf(FATAL, "shm_open gamepad_shm: @client");
			exit(1);
		}
		if ((gp_ptr = mmap(NULL, sizeof(gamepad_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd_shm, 0)) == MAP_FAILED) {
			log_printf(FATAL, "mmap gamepad_shm: @net_handler");
		...
		if (close(fd_shm) == -1) {
			log_printf(ERROR, "close gamepad_shm: @client");
		}
		...
		if ((fd_shm = shm_open(ROBOT_DESC_SHM_NAME, O_RDWR, 0)) == -1) { //no O_CREAT
			log_printf(FATAL, "shm_open robot_desc_shm: @client");
			exit(1);
		}
		if ((rd_ptr = mmap(NULL, sizeof(robot_desc_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd_shm, 0)) == MAP_FAILED) {
			log_printf(FATAL, "mmap robot_desc_shm: @client");
			exit(1);
		}
		if (close(fd_shm) == -1) {
			log_printf(ERROR, "close robot_desc_shm: @client");
		}
...
{	
	...
	if (munmap(gp_ptr, sizeof(gamepad_t)) == -1) {
		(process == NET_HANDLER) ? log_printf(ERROR, "munmap gamepad_shm: @net_handler") : log_printf(ERROR, "munmap gamepad_shm: @client");
	}
	if (munmap(rd_ptr, sizeof(robot_desc_t)) == -1) {
		(process == NET_HANDLER) ? log_printf(ERROR, "munmap robot_desc_shm: @net_handler") : log_printf(ERROR, "munmap robot_desc_shm: @client");
	}
	...
	if (process == NET_HANDLER) {
		...
		if (shm_unlink(GPAD_SHM_NAME) == -1) {
			log_printf(ERROR, "shm_unlink gamepad_shm @net_handler");
		}
		if (shm_unlink(ROBOT_DESC_SHM_NAME) == -1) {
			log_printf(ERROR, "shm_unlink robot_desc_shm @net_handler");
		}
		
		//unlink semaphores
		if (sem_unlink(GP_MUTEX_NAME) == -1) {
			log_printf(ERROR, "sem_unlink gamepad_mutex @net_handler");
		}
		if (sem_unlink(RD_MUTEX_NAME) == -1) {
			log_printf(ERROR, "sem_unlink robot_desc_mutex @net_handler");
		}
	}
}
