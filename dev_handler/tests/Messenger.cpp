...
Status Messenger::send_message (MessageID msg_id, message_t *msg, uint32_t params, uint16_t delay, dev_id_t *uid)
{
    ...
    cobs_buf[1] = (uint8_t) cobs_len;

	if (OUTPUT == USB_DEV) {
		//written = Serial.write(cobs_buf, 2 + cobs_len); //write to serial port
	} else if (OUTPUT == FILE_DEV) {
		written = fwrite(cobs_buf, sizeof(uint8_t), 2 + cobs_len, this->write_file);
        fflush(this->write_file); // Force write the contents to file
	}
...
Status Messenger::read_message (message_t *msg)
{
	...
	if (OUTPUT == USB_DEV) {
		...
		while (!feof(this->read_file)) {
			fread(&last_byte_read, sizeof(uint8_t), 1, this->read_file);
			if (last_byte_read == 0) {
	...
	printf("Making sure cobs len exists\n");
	if (OUTPUT == USB_DEV) {
		...
		if (feof(this->read_file) == 0) {
			return Status::MALFORMED_DATA;
		}
		// Peek at the next character
		int c = fgetc(this->read_file);
		ungetc(c, this->read_file);
		// If next character is 0x00 (start of next message), it's MALFORMED_DATA
		if (c == 0) {
			return Status::MALFORMED_DATA;
		}
	}
	...
	size_t cobs_len;
	if (OUTPUT == USB_DEV) {
		//cobs_len = Serial.read();
	} else if (OUTPUT == FILE_DEV) {
		fread(&cobs_len, sizeof(uint8_t), 1, this->read_file);
	}
	printf("Got cobs len\n");
	// Read the encoded message
	size_t read_len; // The number of bytes actually read. Should == cobs_len
	if (OUTPUT == USB_DEV) {
		//read_len = Serial.readBytesUntil(0x00, (char *)cobs_buf, cobs_len);
	} else if (OUTPUT == FILE_DEV) {
		read_len = fread(cobs_buf, sizeof(uint8_t), cobs_len, this->read_file);
	}
	printf("Read message\n");
