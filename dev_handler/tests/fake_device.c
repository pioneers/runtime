 ...
 */
int receive_message(message_t* relay, message_t* msg) {
 ...
 */
void send_message(message_t* relay, message_t* msg) {
...
void* sender(void* relay_cast) {
    ...
    while (1) {
        if (relay->got_hb_req) {
            ...
            destroy_message(msg);
            relay->got_hb_req = 0;
        } else if ((millis() - relay->sent_hb_req) > HB_REQ_FREQ) {
...
void init() {
    ...
    relay->read_file = fopen(TO_DEVICE, "r");
