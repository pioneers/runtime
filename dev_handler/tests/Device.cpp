...
void Device::loop ()
{
	...
	if (sts == Status::SUCCESS) { //we have a message!
		switch (this->curr_msg.message_id) {
			...
			case MessageID::SUBSCRIPTION_REQUEST:
				...
				this->sub_delay = *((uint16_t *) &(this->curr_msg.payload[4]));
				this->msngr->send_message(MessageID::SUBSCRIPTION_RESPONSE, &(this->curr_msg), params, sub_delay, &UID);
				break;
			...
			case MessageID::DEVICE_READ:
				...
				payload_ptr_uint32 = (uint32_t *) this->curr_msg.payload; //store the pointer to the front of the payload, cast to uint16_t
				*payload_ptr_uint32 = device_rw_all(&(this->curr_msg), *(uint8_t *)payload_ptr_uint32, RWMode::READ); //payload_ptr_uint32 contains bitmap for rw
			...
			case MessageID::DEVICE_WRITE:
				...
				payload_ptr_uint32 = (uint32_t *) this->curr_msg.payload; //store pointer to the front of the payload, cast to uint16_t
				*payload_ptr_uint32 = device_rw_all(&(this->curr_msg), *(uint8_t *)payload_ptr_uint32, RWMode::WRITE);  //payload_ptr_uint32 contains bitmap for rw
	...
	if ((this->sub_delay > 0) && (this->curr_time - this->prev_sub_time >= this->sub_delay)) {
		...
		payload_ptr_uint32 = (uint32_t *) this->curr_msg.payload; //store the pointer to the front of the payload, cast to uint16_t
		*payload_ptr_uint32 = device_rw_all(&(this->curr_msg), this->params, RWMode::READ); //read all subscribed values from device and store in curr_msg
 ...
 */
uint32_t Device::device_rw_all (message_t *msg, uint32_t params, RWMode mode)
{
	int bytes_written = 4; //number of bytes we've written so far into the payload of the current message OR number of bytes successfully written to device
...
{
	payload_val = (payload_val < 100) ? payload_val : 100; //min(payload_val, 100); //don't want the value to be > 100
	...
	float holding = ((Device::MAX_SUB_DELAY_MS * (float) payload_val / 100.0) > Device::MIN_SUB_DELAY_MS) ? \
		(Device::MAX_SUB_DELAY_MS * (float) payload_val / 100.0) : Device::MIN_SUB_DELAY_MS;
	this->sub_delay = (uint16_t)(Device::ALPHA * this->sub_delay + (1.0 - Device::ALPHA) * holding); //set the new sub_delay
}

/* Returns the number of milliseconds since the Unix Epoch */
int64_t Device::millis() {
